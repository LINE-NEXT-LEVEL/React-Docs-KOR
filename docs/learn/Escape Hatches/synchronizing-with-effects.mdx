...
모든 리랜더링 후에 연결을 유지하는 것은 느리기 떄문에 의존성 배열을 추가해야합니다:
...
사용자가 다른 화면으로 이동한다고 생각해보세요- 예를 들어, 설정 화면이요. `Chatroom` 컴포넌트는 내려갈 것입니다. 
...
**`"✅ Connecting..."` 로그를 두 번 보는 것은 진짜 문제를 확이할 수 있도록 도와줍니다: 코드가 컴포넌트가 내려갈 때 연결을 닫지 않고 있다는 것입니다.**
