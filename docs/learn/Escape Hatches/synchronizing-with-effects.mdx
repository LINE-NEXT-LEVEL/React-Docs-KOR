...
렌더링 동안 `ref.current.focus()`를 부르는 것은  `ref.current.focus()`가 *사이드 이펙트*이기 때문에 잘못됬습니다.
...
[엄격 모드](https://beta.reactjs.org/reference/react/StrictMode)가 켜져있을 때(이 사이트에 있는 sandbox 안의 코드처럼), React는 개발환경에서 한 번 더 각각의 컴포넌트를 재마운트합니다.
...
이 코드를 수정하기 위해, `setInterval`에서 반환된 interval ID를 저장하고 [`clearInterval`](https://developer.mozilla.org/en-US/docs/Web/API/clearInterval)를 이용한 제거함수를 실행합니다.

<Code src="https://codesandbox.io/s/o25bp3?file=%2FCounter.js&utm_medium=sandpack" />

개발환경에서, React는 여러분이 제거를 잘 실행했는지 확인하기 위해 컴포넌트를 한 번 더 재마운트할 것입니다.
...
만약 Effect가 비동기적으로 무언가를 가져온다면, 보통 제거함수가 필요합니다.
...
이 경쟁상태를 고치기 위해서 제거함수를 추가해야 합니다.
