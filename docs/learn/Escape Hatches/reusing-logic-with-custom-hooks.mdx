...
:::info Deep Dive
# Custom hook이 높은 수준의 용례들에 대해 집중하도록 하세요
...
이상적으로 여러분의 custom hook의 이름은 코드를 한 줄도 쓸줄 모르는 사람이 보더라도 custom hook이 무엇을 하고 무엇을받고,
...
**Custom hook이 높은 수준의 용례들에 집중할 수 있게 해주세요.** Custom hook이 단순히 `useEffect` API를 감싸거나 이의 대안 정도밖에 안되는
"생명주기" hook이 되어서는 안됩니다.

- 🔴 useMount(fn)
- 🔴 useEffectOnce(fn)
- 🔴 useUpdateEffect(fn)

예를 들어, 이 `useMount` hook은 어떤 코드가 "마운트 시점에" 돌아갈 수 있도록 해줍니다.
...
만약 여러분의 custom hook API가 용례를 제한하지 않고 매우 추상적이라면, 장기적으로는 풀기 어려운 문제들을 많이 만들 것입니다.
