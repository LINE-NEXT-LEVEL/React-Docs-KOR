...
- 종속성을 제거하려면 링터에 의존성이 필요하지 않다는 것을 "증명"해야 합니다.
...
이 효과는 초당 눈금이 표시되는 인터벌을 설정합니다. 여러분은 이상한 일이 일어나는 것을 알아차렸습니다. 
인터벌이 똑딱거릴 때마다 파괴되고 다시 생성되는 것 같습니다. 
...
그러나 이로 인해 Effect는 매초마다 변경되는 `count`에 따라 달라지므로 매초마다 안터벌이 다시 만들어집니다.
...
이 예에서 "표시"를 누르면 시작 메시지가 사라집니다. 
애니메이션은 몇 초 정도 걸립니다. 
제거를 누르면 시작 메시지가 즉시 사라집니다. 
...
`onApear`와 같은 Effect Events는 반응성이 없으므로 애니메이션을 다시 트리거하지 않고 내부의 지속 시간을 읽을 수 있습니다.

### 도전3. 채팅 재연결 고치기

이 예에서는 "테마 토글"을 누를 때마다 채팅이 다시 연결됩니다. 
왜 이런 일이 일어나나요? 
...
이 예에서는 테마를 변경할 때마다 채팅이 다시 연결됩니다. 이거 고쳐주세요. 수정 후 테마를 변경하면 대화가 다시 연결되지 않지만 암호화 설정을 전환하거나 대화방을 변경하면 다시 연결됩니다.
...
두 가지 함수, 즉 `onMessage` 및 `createConnection`를 전달하는 것입니다. 둘 다 `App` 재렌더링 시마다 처음부터 생성됩니다. 
이러한 값은 매번 새로운 값으로 간주되므로 Effect를 다시 유발합니다.

이러한 함수 중 하나는 이벤트 처리기입니다. 이벤트 핸들러 함수의 새 값에 "반응"하지 않고 이벤트 핸들러를 Effect라고 부르는 방법을 알고 계시나요? 그것은 도움이 될 것입니다!

이러한 함수 중 다른 함수는 가져온 API 메서드에 일부 상태를 전달하는 경우에만 존재합니다. 
이 기능이 정말 필요합니까? 전해져야하는 필수 정보는 무엇인가요? 일부 import를 `App.js`에서 `ChatRoom.js`로 이동해야 할 수 있습니다.
...
따라서 Effect의 종속성일 필요가 없습니다. 따라서 `onMessage`로 변경해도 채팅이 다시 연결되지 않습니다.

`createConnection`은 반응형이어야 _하므로_ 동일한 작업을 수행할 수 없습니다. 
사용자가 암호화된 연결과 암호화되지 않은 연결을 전환하거나 현재 채팅방을 전환할 경우 Effect를 다시 트리거하길 _원할 수_ 있습니다. 
그러나 `createConnection`은 함수이므로 읽기 정보가 _실제로_ 변경되었는지 여부를 확인할 수 없습니다. 
