...
Effect를 작성할 때, Effect가 해야 할 일이 무엇이든 시작하고 멈추는 방법을 기술합니다.
...
반응형 값은 컴포넌트 내부에 바로 선언된 모든 변수들과 함수들 그리고 props을 포함합니다.
...
Effect 코드에서 사용되는 모든 반응형 값들의 배열로 의존성 배열을 생각할 수 있습니다.
...
작성해놓은 코드가 있다면, 여러분은 몇몇 Effect가 아래와 같이 linter를 제한하는 코드를 가지고 있을지 모릅니다.
...
간격은 오직 처음 한번만 서렂ㅇ된다는 것을 확실히 하기 위해, [`onTick`을 Effect Event로 만듭니다](https://react.dev/learn/separating-events-from-effects#reading-latest-props-and-state-with-effect-events).
...
마지막 코드는 처음 코드보다 더 길지만 Effect를 분리하는 것이 더 정확합니다.
