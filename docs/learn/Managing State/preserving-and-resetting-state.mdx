...
상태는 컴포넌트에 담긴 채로 격리되어 있습니다. React는 UI 트리 내부의 자신의 위치에  기반해 어떤 상태가 어떤 컴포넌트의 것인지를 끊임 없이 추적합니다.
...
그치만 상태는 실제로 각 컴포넌트가 아닌 React가 들고 있습니다. React는 컴포넌트가 UI 트리 상에서 어디 놓여있는 지와 들고 있는 상태 조각들을 연관짓습니다.
...
**많은 앱들에서 이건 보통 요구되는 동작일 겁니다. 그치만 이 앱에서는 아닙니다!** 유저가 입력한 메시지를 클릭 실수로 잘못된 사람에게 보내지는 것을 바라지 않을 것입니다.
...
- React 상태와 별개의 저장소를 이용하는 방법이 있습니다. 갑자기 페이지가 닫히더라도 메시지 초안이 사라지지 않도록 하는 처리를 해야 한다고 가정해 봅시다.
...
그치만 이 방법은 훨씬 명확하지 않은 방법이고 누군가 `null`을 지운다면 위험해질 수 있는 방법입니다.
...
입력창의 순서가 바뀔때 입력된 내용_도_ 함께 바뀌도록 코드를 수정해보세요.
...
그치만 우리가 원하는 것은 상태가 _특정한 연락처_와 연결되는 것이죠.
