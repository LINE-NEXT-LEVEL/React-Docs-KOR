...
`useReducer` 훅은 `useState`와 비슷합니다. - 초기 상태를 전달해야하고 useReducer가 상태로 쓰이는 값과 그 상태를 변경하는 함수를 돌려줍니다(이 경우에는 dispatch 함수입니다). 
하지만 조금 다른 부분이 있습니다.
...
1. 상태로 사용되는 값
2. dispatch 함수 (사용자의 action들을 reducer에 "보내기"위한 함수)
...
내부적으로 Immer는 `초안`에 만든 변경 상태의 복사본을 만들 것입니다. 그렇기 때문에 `useImmerReudcer`로 관리되는 reducer들은 첫 번째 인자를 변경할 수 있고 상태를 반환하지 않아도 됩니다. 
...
### 도전 1: 벤트 핸들러에서 action 보내기

현재, `ContactList.js`와`Chat.js`안의 이벤트 핸들러들은`// TODO`코멘트를 갖고 있습니다. 입력이 작동하지 않는 이유이고, 버튼 클릭이 선택 참여자 이름을 바꾸지 않는 이유입니다.
...
### 도전2 :메세지를 보내면서 입력창 초기화하기 

현재, "보내기" 버튼을 누르는 것은 아무것도 하지 않습니다. "보내기" 버튼에 이벤트핸들러를 추가하는 것은 다음과 같은 일을 할 것입니다:

1. 받는 사람의 이메일과 메세지와 함께 'alert' 띄우기
2. 입력창 초기화하기. 

<Code src="https://codesandbox.io/s/gkinsn?file=%2FChat.js&utm_medium=sandpack"/>

### 정답 

"보내기" 버튼 이벤트 핸들러에서 이 작업을 수행할 수 있는 몇 가지 방법이 있습니다. 
한 가지 접근 방식은 경고창를 표시한 다음 빈 `message`와 함께 `edited_message` 액션을 보내는 것입니다:

<Code src="https://codesandbox.io/s/589v8h?file=%2FChat.js&utm_medium=sandpack"/>

이제 동작하고 "보내기"를 누르면 입력창이 초기화됩니다. 

그러나 *사용자의 관점에서* 메시지를 보내는 것은 필드를 편집하는 것과는 다른 작업입니다. 
이를 반영하기 위해 대신 `sent_message`라는 *새* 작업을 생성하고 reducer에서 별도로 처리할 수 있습니다:

<Code src="https://codesandbox.io/s/k7yl8v?file=%2FmessengerReducer.js&utm_medium=sandpack"/>

결과 동작은 동일합니다. 그러나 작업 유형은 "상태 변경 방법"이 아니라 "사용자가 수행한 작업"을 이상적으로 설명해야 합니다. 
이렇게 하면 나중에 기능을 더 쉽게 추가할 수 있습니다.

두 해법 중 하나를 사용할 경우 경보창을 reducer 내부에 배치하지 않는 것이 중요합니다. 
reducer는 순수 함수여야 하며 다음 상태만 계산해야 합니다. 
사용자에게 메시지를 표시하는 것을 포함하여 아무것도 "하지" 않아야 합니다. 
경고창을 띄우는 것은 이벤트 핸들러에서 발생해야 합니다. 
(이와 같은 실수를 방지하기 위해 React는 엄격한 모드에서 여러 번 reducer를 호출합니다.
이 때문에 경고창을 reducer에 넣으면 경고창이 두 번 실행됩니다.)

### 도전 3: 탭을 전환할 때 입력값 보존하기

이 예에서, 수신자를 변경하면 매 번 텍스트 입력창이 초기화됩니다:
...
이는 여러 수신인 간에 단일 메시지 초안을 공유하지 않기 때문입니다. 
하지만 앱이 각 연락처에 대한 초안을 별도로 "기억"하여 연락처를 전환할 때 해당 연락처를 복원하는 것이 더 나을 수 있습니다.

연락처별로 별도의 메시지 초안을 기억할 수 있도록 상태가 구성되는 방식을 변경하는 것이 작업입니다. reducer, 초기 상태 및 컴포넌트를 몇 가지 변경해야 합니다.

<Code src="https://codesandbox.io/s/k7yl8v?file=%2FApp.js&utm_medium=sandpack"/>
...
`[key]: value` [계산된 속성](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Object_initializer#computed_property_names) 문법이 `messages` 객체를 업데이트하도록 도와줄 것입니다.

### 해답

연락처마다 각각의 메세지 초안을 저장하고 업데이트하도록 reducer를 업데이트할 필요가 있을 것입니다. 
...
현재 선택된 연락처를 위한 메세지를 읽기 위해 `Messenger` 컴포넌트도 업데이트 해야합니다. 
...
<Code src="https://codesandbox.io/s/emzq56?file=%2FApp.js&utm_medium=sandpack"/>

특히 이러한 다른 동작을 구현하기 위해 이벤트 핸들러를 변경할 필요가 없습니다. 
reducer가 없으면 상태를 업데이트하는 모든 이벤트 핸들러를 변경해야 할 것입니다.

### 도전4: `useReducer`를 처음부터 구현해보기

이전 예시에서, React에서 `useReducer` 훅을 가져와서 사용했습니다.
이번에는. `useReducer` 훅 그 자체를 구현해볼 것입니다! 여기 시작해볼 수 있는 뼈대가 있습니다. 
10줄 넘게 필요하지 않을 것입니다.

변경을 테스트해보기 위해, 입력창에 입력해보거나 연락처를 선택해보세요. 

<Code src="https://codesandbox.io/s/e9dbk6?file=%2FMyReact.js&utm_medium=sandpack"/>
...
두 개의 인자를 받는 reducer 함수를 다시 불러보세요. 두 개의 인자는 현재의 상태와 action 객체입니다. 
그리고 함수는 다음 상태를 반환합니다. `dispatch`는 반환된 상태로 무엇을 하도록 구현이 되어야 할까요?

### 해답 

action을 보내는 것은 현재의 상태와 action을 가진 reducer를 호출합니다. 그리고 결과값과 다음 상태를 저장합니다. 코드는 다음과 같이 생겨야합니다.

<Code src="https://codesandbox.io/s/y512l0?file=%2FMyReact.js&utm_medium=sandpack"/>

대부분의 경우에 중요하지 않지만, 조금 더 정확하게 구현하면 다음과 같습니다. 
...
보내진 action은 다음 렌더링 때까지 큐 형태로 쌓이기 때문에, [업데이터 합수와 비슷](https://react.dev/learn/queueing-a-series-of-state-updates)합니다. 